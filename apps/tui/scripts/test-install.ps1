...
$releaseDir = Join-Path $fixtures $tag
$archivePath = Join-Path $releaseDir $archiveName
$server = $null

try {
    New-Item -ItemType Directory -Force -Path $fixtures, $package, $releaseDir | Out-Null
    ...
    Set-Content -LiteralPath (Join-Path $releaseDir "SHA256SUMS") -Value "$hash  $archiveName"
